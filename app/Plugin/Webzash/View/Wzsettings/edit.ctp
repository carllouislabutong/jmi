...
<?php
	...
	echo $this->Form->input('sitename', array(
		...
		'afterInput' => '<span class="">' . __d('webzash', 'Note : Sitename to use in all user registration related emails.') . '</span>',
	));
	echo $this->Form->input('drcr_toby', array(
		'type' => 'select',
		'options' => $this->Generic->wzsetting_drcr_toby_options(),
		'label' => __d('webzash', 'In entries use'),
		'afterInput' => '<span class="text-warning">' . __d('webzash', 'Note : Whether to use Dr/Cr or To/By in entries.') . '</span>',
	));
	echo $this->Form->input('enable_logging', array(
		'type' => 'checkbox',
		'label' => __d('webzash', 'Enable logging'),
		'class' => 'checkbox',
		'afterInput' => '<span class="text-warning">' . __d('webzash', 'Note : Log changes to the accounts which can be seen in the account dashboard.') . '</span>',
	));
	echo $this->Form->input('row_count', array(
		'type' => 'select',
		'options' => $this->Generic->wzsetting_row_count_options(),
		'label' => __d('webzash', 'Row count'),
		'afterInput' => '<span class="text-warning">' . __d('webzash', 'Note : Number of rows to show at a time.') . '</span>',
	));
	...
	echo "<fieldset><legend style='color: white'>Outgoing Email</legend>";
